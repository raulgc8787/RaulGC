...
class WPAlchemy_MetaBox
{
	...
	function _get_post_id()
	{
		...
		if ( is_null($post_id) ) $post_id = isset($post->ID) ? $post->ID : $post_id ;
	...
	function _global_head()
	{
		...
		jQuery(function($)
		{
			...
			$('[class*=docopy-]').click(function(e)
			{
				...
				var the_group = $('.wpa_group-'+ the_name +'.last.tocopy', p);
				...
				if (vals_clone) {
					...
					var $class = $('.thumbcontainer input[type="text"]', the_clone).attr('class');
					var inputfield = $('.thumbcontainer input[type="text"]', the_clone);
					
					inputfield.attr('value', vals_clone.id).attr('data', vals_clone.url);
		
					if (vals_clone.type == 'image') inputfield.after('<img class="'+$class+'" src="'+$.getThumbUrl(vals_clone.id)+'" />');
					
					$('.caption textarea', the_clone).val(vals_clone.caption);

					var $remove = the_clone.find('.removeimg');
					$remove.append('<a href="#" data="'+$class+'">Remove</a>').click(function()
					{	
						$('a', this).remove();
						$('input.'+$class).val("");
			...
			/* Added feature multiple uploader for WP 3.5 */
			...
			var set_to_post_id = 0; // Set this
			  ...
			  jQuery('.upload_image_button').live('click', function( event ){
			    ...
			    file_frame = wp.media.frames.file_frame = wp.media({
			      title: 'Choose single/multiple files',
			      button: {
			    ...
			    file_frame.on( 'select', function() {
			    	...
			    	selection.map( function( attachment ) {
				    	...
				    	vals_clone = attachment;
				    	
				    	$('[class*=docopy-]').trigger('click');
				...
				jQuery(document).on('click', '.add_image_button', function(event) {
				//jQuery('.add_image_button').live('click', function( event ){
				 
				 	var el = $(this).closest('.slidebox');
				 	var input = jQuery(this).parent();
					event.preventDefault();

					if (el[0] == undefined) el = $(this).prev('.bgimage');
			    ...
			    file_frame.on( 'select', function() {
					...
					var inputfield = $('.thumbcontainer input[type="text"]', el);

					if (el.hasClass('bgimage')) el.val(attachment.url);
				    	if (attachment.type == 'image') inputfield.after('<img class="'+$class+'" src="'+$.getThumbUrl(attachment.id)+'" />');
				    	var $class = $('.addbutton input', el).attr('class');
				    	$('.addbutton input', el).val(attachment['id']);
				    	$('.addbutton input', el).after('<img class="'+$class+'" src="'+attachment['sizes']['thumbnail']['url']+'" />');
				    	$('.caption textarea', el).val(attachment['caption']);

					var $remove = el.find('.removeimg');
					$remove.append('<a href="#" data="'+$class+'">Remove</a>').click(function()
					{	
						$('a', this).remove();
						$('input.'+$class).val("");
