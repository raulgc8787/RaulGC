<?php if ( ! defined( 'EWPT_PLUGIN_VER_MOD') ) exit( 'No direct script access allowed' );
/**
...
class Envato_Backup_Mod {
  ...
  public function backup() {
    ...
    do_action( 'Envato_Backup_Mod_started', $this );
  
    /* Zip everything up */
    $this->archive();
  
    do_action( 'Envato_Backup_Mod_complete', $this );
  ...
  public function pcl_zip() {
    ...
    global $_Envato_Backup_Mod_exclude_string;
    
    $_Envato_Backup_Mod_exclude_string = $this->exclude_string( 'regex' );
    ...
    /* Zip up everything */
    if ( ! $archive->add( $this->root(), PCLZIP_OPT_REMOVE_PATH, $this->root(), PCLZIP_CB_PRE_ADD, 'Envato_Backup_Mod_pclzip_callback_mod' ) )
      $this->warning( $this->archive_method, $archive->errorInfo( true ) );
    
    unset( $GLOBALS['_Envato_Backup_Mod_exclude_string'] );
 ...
 * @return    bool
 */
function Envato_Backup_Mod_pclzip_callback_mod( $event, &$file ) {
  ...
  elseif ( $_Envato_Backup_Mod_exclude_string && preg_match( '(' . $_Envato_Backup_Mod_exclude_string . ')', $file['stored_filename'] ) )
  return false;
