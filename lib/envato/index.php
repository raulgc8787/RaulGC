 ...
 */
class Envato_WP_Toolkit_modified {
  /**
  ...
  protected function _constants() {
     ...
     * Plugin Version
     */
    define( 'EWPT_PLUGIN_VER_MOD', '1.4' );
    
    /**
     * Plugin Name
     */
    define( 'EWPT_PLUGIN_NAME_MOD', __( 'Envato WordPress Toolkit', 'envato' ) );
    
    /**
     * Plugin Slug
     */
    define( 'EWPT_PLUGIN_SLUG_MOD', 'envato-wordpress-toolkit' );
    
    /**
     * Maximum request time
     */
    define( 'EWPT_PLUGIN_MAX_EXECUTION_TIME_MOD' , 60 * 5);
    
    /**
     * Plugin Directory Path
     */
    define( 'EWPT_PLUGIN_DIR_MOD', plugin_dir_path( __FILE__ ) );
    
    /**
     * Plugin Directory URL
     */
    define( 'EWPT_PLUGIN_URL_MOD', plugin_dir_url( __FILE__ ) );
    
    /**
     * Theme Backup Directory Path
     */
    define( 'EWPT_BACKUP_DIR_MOD', WP_CONTENT_DIR . '/envato-backups/' );

    /**
     * Theme Backup Directory URL
     */
    define( 'EWPT_BACKUP_URL_MOD', WP_CONTENT_URL . '/envato-backups/' );
     ...
     */
    define( 'EWPT_SECURE_KEY_MOD', md5( NONCE_KEY ) );
  }
  ...
  protected function _includes() {
    ...
    foreach ( array( 'class-envato-api', 'class-wp-upgrader', 'class-envato-backup' ) as $file )
      require_once( EWPT_PLUGIN_DIR_MOD . 'includes/' . $file . '.php' );
  }
  ...
  public function _envato_menu() {
    $menu_page = add_submenu_page( 'undsgnoptions', EWPT_PLUGIN_NAME_MOD, '<span class="undsgn-update">' . __( 'Updates', 'envato' ). '</span>', 'manage_options', EWPT_PLUGIN_SLUG_MOD, array( &$this, '_envato_menu_page' ), EWPT_PLUGIN_URL_MOD . 'assets/images/envato.png', 59 );
  ...
  public function _envato_load_styles() {
    wp_enqueue_style( 'envato-wp-updater', EWPT_PLUGIN_URL_MOD . 'assets/css/style.css', false, EWPT_PLUGIN_VER_MOD, 'all' );
  }
  ...
  public function _envato_menu_page() {
    ...
    $options = get_option( EWPT_PLUGIN_SLUG_MOD );

    $user_name = ( isset( $options['user_name'] ) ) ? $options['user_name'] : '';
    $api_key = ( isset( $options['api_key'] ) ) ? $options['api_key'] : '';
    
    $this->protected_api =& new Envato_Protected_API_Mod( $user_name, $api_key );
    ...
    /* display normal views */
      ...
      echo '<div class="wrap">';
        echo '<div id="icon-themes" class="icon32"></div><h2>' . EWPT_PLUGIN_NAME_MOD . '</h2>';
        echo '
        <form name="verification_form" method="post" action="' . admin_url( 'options.php' ) . '" id="api-verification">';
          wp_nonce_field( 'update-options' );
          settings_fields( EWPT_PLUGIN_SLUG_MOD );
          do_settings_sections( EWPT_PLUGIN_SLUG_MOD );
          echo '
          <p class="submit">
        ...
        if ( empty( $errors ) && count( $themes ) > 0 ) {
          ...
          foreach( $themes as $theme ) {
           ...
           if($theme->author_name == 'undsgn.com') {
	            /* setup the defaults */
	            $content = '';
	            $installed = false;
	            $links = array();
	            ...
	            $activate_url = wp_nonce_url( network_admin_url( 'admin.php?page=' . EWPT_PLUGIN_SLUG_MOD . '&action=activate&amp;template=' . urlencode( $template ) . '&amp;stylesheet=' . urlencode( $stylesheet ) ), 'switch-theme_' . $template );
	            ...
	            $delete_url = wp_nonce_url( network_admin_url( 'admin.php?page=' . EWPT_PLUGIN_SLUG_MOD . '&action=delete&template=' . $stylesheet ), 'delete-theme_' . $stylesheet );
	            ...
	            $install_url = wp_nonce_url( network_admin_url( 'admin.php?page=' . EWPT_PLUGIN_SLUG_MOD . '&action=install-theme&theme=' . $item_id ), 'install-theme_' . $item_id );
	            $update_url = wp_nonce_url( network_admin_url( 'admin.php?page=' . EWPT_PLUGIN_SLUG_MOD . '&action=upgrade-theme&amp;theme=' . $stylesheet . '&amp;item_id=' . $item_id ), 'upgrade-theme_' . $stylesheet );
	            ...
	            if ( $has_update ) {
	              ...
	              ?>
					...
					//<![CDATA[
						jQuery(document).ready( function($) {
							jQuery('.undsgn-update').html('<span class="undsgn-update">Updates <span class="update-plugins count-1"><span class="update-count">New</span></span></span>');
					    });
					//]]>
					</script>
					<?php
	              ...
	              if (get_option('notified') != "yes") {
				
					$pointer_content = '<h3>Studiofolio update available!</h3>';
					$pointer_content .= '<p>A new update of the Studiofolio theme <br /> is now available to download';
					?>
					<script type="text/javascript">
					//<![CDATA[
						jQuery(document).ready( function($) {
							jQuery('#toplevel_page_undsgnoptions').pointer({
	              ...
	              if ( ! current_user_can( 'update_themes' ) ) {
	                ...
	                $content.= sprintf( '<div class="updated below-h2"><p><strong>' . __( 'There is a new version of %1$s available. <a href="%2$s" class="thickbox thickbox-preview" title="%1$s">View version %3$s details</a> or <a href="%4$s" %5$s>update automatically</a>. <span style="color: red;">This will erase eventual code personalisations, so make a backup of the folder /themes/'.$title.'</span>', 'envato' ) . '</strong></p></div>', $title, $details_url, $latest_version, $update_url, $update_onclick );
	              }
	            } else if ( ! $installed ) {
	              if ( ! current_user_can( 'update_themes' ) ) {
	            ...
	            } else update_option( 'notified', 'no' );  
          ...
          if ( ! empty( $premium_themes ) ) {
            ...
            /* list premium themes */
            _e( '<h3>Available Themes from Undsgn.com</h3>' );
            echo '<ul class="ewpu-item-list">';
            foreach ( $premium_themes as $k => $v )
  ...
  protected function _admin_init_before() {
    if ( isset( $_GET['page'] ) ) {
      ...
      if ( $page == EWPT_PLUGIN_SLUG_MOD ) {
        
        $this->_prepare_Envato_Backup_Mod();
  ...
  public function _admin_init() {
    ...
    register_setting( EWPT_PLUGIN_SLUG_MOD, EWPT_PLUGIN_SLUG_MOD );
    add_settings_section( 'user_account_info', __( 'User Account Information', 'envato' ), array( &$this, '_section_user_account_info' ), EWPT_PLUGIN_SLUG_MOD );
    add_settings_field( 'user_name', __( 'Marketplace Username', 'envato' ), array( &$this, '_field_user_name' ), EWPT_PLUGIN_SLUG_MOD, 'user_account_info' );
    add_settings_field( 'api_key', __( 'Secret API Key', 'envato' ), array( &$this, '_field_api_key' ), EWPT_PLUGIN_SLUG_MOD, 'user_account_info' );
    //add_settings_section( 'backup_info', __( 'Backup Information', 'envato' ), array( &$this, '_section_backup_information' ), EWPT_PLUGIN_SLUG_MOD );
    //add_settings_field( 'skip_theme_backup', __( 'Skip Theme Backup', 'envato' ), array( &$this, '_field_skip_theme_backup' ), EWPT_PLUGIN_SLUG_MOD, 'backup_info' );
  }
  ...
  public function _field_user_name() {
    $options = get_option( EWPT_PLUGIN_SLUG_MOD );
    echo '<input type="text" class="regular-text" name="' . EWPT_PLUGIN_SLUG_MOD . '[user_name]" value="' . esc_attr( $options['user_name'] ) . '" />';
  }
  ...
  public function _field_api_key() {
    $options = get_option( EWPT_PLUGIN_SLUG_MOD );
    echo '<input type="password" class="regular-text" name="' . EWPT_PLUGIN_SLUG_MOD . '[api_key]" value="' . esc_attr( $options['api_key'] ) . '" />';
  }
  ...
  public function _field_skip_theme_backup() {
    ...
    echo '<input type="checkbox" class="regular-text" name="' . EWPT_PLUGIN_SLUG_MOD . '[skip_theme_backup]" value="1" ' . checked( $field_value, 1, false ) . ' />';
  }
    ...
    if ( isset( $_GET['page'] ) && isset( $_GET['action'] ) ) {
      ...
      if ( $page == EWPT_PLUGIN_SLUG_MOD ) {
        if ( 'install-theme' == $action || 'upgrade-theme' == $action ) {
          $actions['themes_page'] = '<a href="' . network_admin_url( 'admin.php?page=' . EWPT_PLUGIN_SLUG_MOD ) . '" title="' . esc_attr__( sprintf( __( 'Return to %s', 'envato' ), EWPT_PLUGIN_NAME_MOD ) ) . '" target="_parent">' . sprintf( __( 'Return to %s', 'envato' ), EWPT_PLUGIN_NAME_MOD ) . '</a>';
        }
      }
    }
    return $actions;
  ...
  protected function _install_theme( $theme, $themes ) {
    ...
    $url = network_admin_url( 'admin.php?page=' . EWPT_PLUGIN_SLUG_MOD . '&action=install-theme&theme=' . $theme );
    $type = 'web';
    
    /* trick WP into thinking it's the themes page for the icon32 */
    $current_screen->parent_base = 'themes';
    
    /* new Envato_Theme_Upgrader_Mod */
    $upgrader = new Envato_Theme_Upgrader_Mod( new Theme_Installer_Skin( compact( 'title', 'url', 'api', 'nonce' ) ) );
  ...
  protected function _activate_theme( $template, $stylesheet ) {
    ...
    wp_redirect( network_admin_url( 'admin.php?page=' . EWPT_PLUGIN_SLUG_MOD . '&activated=true' ) );
    exit;
  }
  ...
  protected function _upgrade_theme( $theme, $item_id ) {
    ...
    $url = network_admin_url( 'admin.php?page=' . EWPT_PLUGIN_SLUG_MOD . '&action=upgrade-theme&theme=' . $theme . '&item_id=' . $item_id );
   ...
   // if ( isset( $options['skip_theme_backup'] ) || $this->_backup_theme( $theme ) === true ) {
      /* new Envato_Theme_Upgrader_Mod */
      $upgrader = new Envato_Theme_Upgrader_Mod( new Theme_Upgrader_Skin( compact( 'title', 'nonce', 'url', 'theme' ) ) );
  ...
  protected function _delete_theme( $template ) {
    ...
    delete_theme( $template, wp_nonce_url( network_admin_url( 'admin.php?page=' . EWPT_PLUGIN_SLUG_MOD . '&action=delete&template=' . $template ), 'delete-theme_' . $template ) );
    wp_redirect( network_admin_url( 'admin.php?page=' . EWPT_PLUGIN_SLUG_MOD . '&deleted=true' ) );
    exit;
  }
   ...
   * @return    void
   */
  function _prepare_Envato_Backup_Mod() {
    
    $path = EWPT_BACKUP_DIR_MOD;
    ...
    $contents[] = 'RewriteCond %{QUERY_STRING} !key=' . md5( EWPT_SECURE_KEY_MOD );
  ...
  protected function _get_theme_backup_dir( $theme ) {
    ...
    $backup_path = EWPT_BACKUP_DIR_MOD;
  ...
  protected function _get_theme_backup_uri( $theme ) {
    ...
    $theme_backup_uri = str_replace( EWPT_BACKUP_DIR_MOD, EWPT_BACKUP_URL_MOD, $theme_backup );
    
    if ( defined( 'EWPT_SECURE_KEY_MOD' ) ) {
      $theme_backup_uri = $theme_backup_uri . '?key=' . md5( EWPT_SECURE_KEY_MOD );
    }
  ...
  public function _http_request_args( $r ){
    if ( (int) ini_get( 'max_execution_time' ) <  EWPT_PLUGIN_MAX_EXECUTION_TIME_MOD ) {
      try {
          set_time_limit( EWPT_PLUGIN_MAX_EXECUTION_TIME_MOD );
      } catch (Exception $e) {
    ...
    $r['timeout'] = EWPT_PLUGIN_MAX_EXECUTION_TIME_MOD;
    return $r;
  }
 ...
 * @global    object
 */
$envato_wp_toolkit = new Envato_WP_Toolkit_modified();
