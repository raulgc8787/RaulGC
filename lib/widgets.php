...
function studiofolio_widgets_init() {
  // Register widgetized areas
  register_sidebar(array(
    'name' => __('Primary Sidebar', 'studiofolio'),
    'id' => 'sidebar-primary',
    'before_widget' => '<section id="%1$s" class="widget %2$s span12"><div class="widget-inner">',
    'after_widget' => '</div></section>',
    'before_title' => '<h3>',
    'after_title' => '</h3>',
  ));
  register_sidebar(array(
    'name' => __('Footer', 'studiofolio'),
    'id' => 'sidebar-footer',
    'before_widget' => '<section id="%1$s" class="widget %2$s"><div class="widget-inner span2">',
    'after_widget' => '</div></section>',
    'before_title' => '<h3>',
    'after_title' => '</h3>',
  ));
  ...
  // Register widgets
  register_widget('Studiofolio_Vcard_Widget');
}
add_action('widgets_init', 'studiofolio_widgets_init');

// Example vCard widget
class Studiofolio_Vcard_Widget extends WP_Widget {
  function Studiofolio_Vcard_Widget() {
    $widget_ops = array('classname' => 'widget_studiofolio_vcard', 'description' => __('Use this widget to add a vCard', 'studiofolio'));
    $this->WP_Widget('widget_studiofolio_vcard', __('Studiofolio: vCard', 'studiofolio'), $widget_ops);
    $this->alt_option_name = 'widget_studiofolio_vcard';
  ...
  function widget($args, $instance) {
    $cache = wp_cache_get('widget_studiofolio_vcard', 'widget');
    ...
    $title = apply_filters('widget_title', empty($instance['title']) ? __('vCard', 'studiofolio') : $instance['title'], $instance, $this->id_base);
  ...
  <?php
    ...
    wp_cache_set('widget_studiofolio_vcard', $cache, 'widget');
  }
  ...
  function update($new_instance, $old_instance) {
    ...
    if (isset($alloptions['widget_studiofolio_vcard'])) {
      delete_option('widget_studiofolio_vcard');
    }
  ...
  function flush_widget_cache() {
    wp_cache_delete('widget_studiofolio_vcard', 'widget');
  }
  ...
  ?>
    <p>
      <label for="<?php echo esc_attr($this->get_field_id('title')); ?>"><?php _e('Title (optional):', 'studiofolio'); ?></label>
      <input class="widefat" id="<?php echo esc_attr($this->get_field_id('title')); ?>" name="<?php echo esc_attr($this->get_field_name('title')); ?>" type="text" value="<?php echo esc_attr($title); ?>" />
    </p>
    <p>
      <label for="<?php echo esc_attr($this->get_field_id('street_address')); ?>"><?php _e('Street Address:', 'studiofolio'); ?></label>
    ...
    <p>
      <label for="<?php echo esc_attr($this->get_field_id('locality')); ?>"><?php _e('City/Locality:', 'studiofolio'); ?></label>
    ...
    <p>
      <label for="<?php echo esc_attr($this->get_field_id('region')); ?>"><?php _e('State/Region:', 'studiofolio'); ?></label>
    ...
    <p>
      <label for="<?php echo esc_attr($this->get_field_id('postal_code')); ?>"><?php _e('Zipcode/Postal Code:', 'studiofolio'); ?></label>
    ...
    <p>
      <label for="<?php echo esc_attr($this->get_field_id('tel')); ?>"><?php _e('Telephone:', 'studiofolio'); ?></label>
    ...
    <p>
      <label for="<?php echo esc_attr($this->get_field_id('email')); ?>"><?php _e('Email:', 'studiofolio'); ?></label>
